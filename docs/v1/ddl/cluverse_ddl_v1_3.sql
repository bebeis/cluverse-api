...
-- 1.4 student_verification (학생 인증 현재 상태)
CREATE TABLE student_verification (
    student_verification_id BIGINT      NOT NULL AUTO_INCREMENT,
    member_id               BIGINT      NOT NULL                  COMMENT '인증 대상 회원 → member.member_id',
    university_id           BIGINT      NOT NULL                  COMMENT '인증 기준 학교 → university.university_id',
...
CREATE INDEX idx_student_verification_university_id
    ON student_verification (university_id);

CREATE INDEX idx_student_verification_status
    ON student_verification (status);

-- 1.5 student_verification_email_challenge (학교 이메일 인증 시도)
CREATE TABLE student_verification_email_challenge (
    student_verification_email_challenge_id BIGINT       NOT NULL AUTO_INCREMENT,
    student_verification_id                 BIGINT       NOT NULL                  COMMENT '→ student_verification.student_verification_id',
...
CREATE INDEX idx_student_verification_email_challenge_verification_id
    ON student_verification_email_challenge (student_verification_id);

CREATE INDEX idx_student_verification_email_challenge_email
    ON student_verification_email_challenge (email);

CREATE INDEX idx_student_verification_email_challenge_status
    ON student_verification_email_challenge (status);

-- 1.6 member_profile (프로필) — Shared PK
CREATE TABLE member_profile (
    member_id         BIGINT       NOT NULL                     COMMENT 'Shared PK → member.member_id',
    bio               TEXT         NULL,
    entrance_year     INT          NULL                         COMMENT '입학년도',
...
-- 1.9 member_terms_agreement (회원-약관 동의 이력)
...
-- 7.3 notification (프론트 알림)
CREATE TABLE notification (
    notification_id BIGINT       NOT NULL AUTO_INCREMENT,
    member_id       BIGINT       NOT NULL                       COMMENT '수신자 → member.member_id',
    type            VARCHAR(50)  NOT NULL                       COMMENT 'COMMENT / REPLY / GROUP_APPROVED / GROUP_REJECTED / GROUP_ANNOUNCEMENT / FOLLOW / FOLLOWING_POST',
...
-- 7.4 notification_preference (프론트 알림 설정) — Shared PK
...
-- 8.3 member_report (프론트 신고)
CREATE TABLE member_report (
    member_report_id BIGINT      NOT NULL AUTO_INCREMENT,
    reporter_id      BIGINT      NOT NULL                       COMMENT '신고자 → member.member_id',
    target_type      VARCHAR(30) NOT NULL                       COMMENT 'POST / COMMENT / MEMBER / CHAT_MESSAGE / GROUP',
    target_id        BIGINT      NOT NULL,
    reason_code      VARCHAR(30) NOT NULL                       COMMENT 'SPAM / ABUSE / ADULT / FRAUD / COPYRIGHT / ETC',
    detail           TEXT        NULL,
    status           VARCHAR(20) NOT NULL DEFAULT 'RECEIVED'    COMMENT 'RECEIVED / IN_REVIEW / RESOLVED / REJECTED',
    created_at       DATETIME    NOT NULL DEFAULT NOW(),
    updated_at       DATETIME    NOT NULL DEFAULT NOW() ON UPDATE NOW(),
...
CREATE INDEX idx_member_report_reporter_created
    ON member_report (reporter_id, created_at DESC);

-- 8.4 member_report_evidence_image (프론트 신고 첨부 이미지)
CREATE TABLE member_report_evidence_image (
    member_report_id BIGINT       NOT NULL                      COMMENT '→ member_report.member_report_id',
    image_url        VARCHAR(500) NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
  COMMENT='프론트 신고 첨부 이미지';
...
-- 10.2 calendar_event (프론트 캘린더 일정)
CREATE TABLE calendar_event (
    calendar_event_id BIGINT       NOT NULL AUTO_INCREMENT,
    member_id         BIGINT       NOT NULL                     COMMENT '작성자 → member.member_id',
    title             VARCHAR(100) NOT NULL,
    description       TEXT         NULL,
    category          VARCHAR(20)  NOT NULL                     COMMENT 'PERSONAL / GROUP / SCHOOL',
    start_at          DATETIME     NOT NULL,
    end_at            DATETIME     NOT NULL,
    location          VARCHAR(255) NULL,
    all_day           TINYINT(1)   NOT NULL DEFAULT 0,
    visibility        VARCHAR(20)  NOT NULL                     COMMENT 'PRIVATE / MEMBERS / PUBLIC',
    created_at        DATETIME     NOT NULL DEFAULT NOW(),
    updated_at        DATETIME     NOT NULL DEFAULT NOW() ON UPDATE NOW(),
...
CREATE INDEX idx_calendar_event_member_start
    ON calendar_event (member_id, start_at, end_at);

-- 10.3 campus_event (캠퍼스 행사)

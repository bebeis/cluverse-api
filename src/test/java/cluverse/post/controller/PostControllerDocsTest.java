...
class PostControllerDocsTest extends RestDocsSupport {
    ...
    private final PostListQueryService postListQueryService = mock(PostListQueryService.class);
    private final PostService postService = mock(PostService.class);
    private final ViewCountCookieResolver viewCountCookieResolver = mock(ViewCountCookieResolver.class);

    @Override
    protected Object initController() {
        return new PostController(postQueryService, postListQueryService, postService, viewCountCookieResolver);
    }
        ...
        mockMvc.perform(get("/api/v1/posts")
                ...
                .andDo(document("posts/get-post-list",
                        queryParameters(
                                ...
                                parameterWithName("date").description("진입 기준 날짜 (`yyyy-MM-dd`). 커서와 함께 사용할 수 없음").optional(),
                                parameterWithName("cursorCreatedAt").description("응답 커서의 작성 시각 (ISO-8601)").optional(),
                                parameterWithName("cursorPostId").description("응답 커서의 게시글 ID").optional(),
                                parameterWithName("direction").description("이동 방향 (`NEXT` 또는 `PREV`)").optional()
                        ),
                        responseFields(
                                ...
                                fieldWithPath("data.prevCursor").type(JsonFieldType.OBJECT).description("이전 페이지 커서").optional(),
                                fieldWithPath("data.prevCursor.createdAt").type(JsonFieldType.STRING).description("이전 커서 작성 시각").optional(),
                                fieldWithPath("data.prevCursor.postId").type(JsonFieldType.NUMBER).description("이전 커서 게시글 ID").optional(),
                                fieldWithPath("data.nextCursor").type(JsonFieldType.OBJECT).description("다음 페이지 커서").optional(),
                                fieldWithPath("data.nextCursor.createdAt").type(JsonFieldType.STRING).description("다음 커서 작성 시각").optional(),
                                fieldWithPath("data.nextCursor.postId").type(JsonFieldType.NUMBER).description("다음 커서 게시글 ID").optional()
                        )
                ));
    }

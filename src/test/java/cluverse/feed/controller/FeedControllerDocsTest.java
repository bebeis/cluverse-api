...
class FeedControllerDocsTest extends RestDocsSupport {
    ...
    void 홈_피드_조회() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/feeds/home")
                ...
                .andDo(document("feeds/get-home-feed",
                        queryParameters(
                                parameterWithName("filter").description("홈 피드 필터 (`ALL`, `SUBSCRIBED`, `RECOMMENDED`)").optional(),
                                parameterWithName("cursor").description("다음 페이지 조회용 커서").optional(),
                                parameterWithName("limit").description("조회할 게시글 수").optional()
                        ),
                        responseFields(feedPageResponseFields())
                ));
    }
    ...
    void 팔로잉_피드_조회() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/feeds/following")
                        ...
                        .queryParam("cursor", "2026-03-16T12:00:00|10")
                        .queryParam("limit", "20"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.posts[0].board.name").value("AI"))
                .andDo(document("feeds/get-following-feed",
                        queryParameters(
                                parameterWithName("scope").description("팔로잉 피드 범위 (`ALL`, `FOLLOWING_MEMBERS`, `MY_GROUPS`)").optional(),
                                parameterWithName("cursor").description("다음 페이지 조회용 커서").optional(),
                                parameterWithName("limit").description("조회할 게시글 수").optional()
                        ),
                        responseFields(feedPageResponseFields())
    ...
    private FeedPageResponse createFeedPageResponse() {
        return new FeedPageResponse(
                ...
                "2026-03-16T12:00:00|10",
                20,
                true
        );
    }

    private org.springframework.restdocs.payload.FieldDescriptor[] feedPageResponseFields() {
        return new org.springframework.restdocs.payload.FieldDescriptor[]{
                fieldWithPath("code").type(JsonFieldType.NUMBER).description("HTTP 상태 코드"),
                fieldWithPath("status").type(JsonFieldType.STRING).description("HTTP 상태"),
                fieldWithPath("message").type(JsonFieldType.STRING).description("메시지"),
                fieldWithPath("data.posts").type(JsonFieldType.ARRAY).description("피드 게시글 목록"),

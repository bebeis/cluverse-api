...
class HomeControllerV2DocsTest extends RestDocsSupport {

    private final HomeQueryService homeQueryService = mock(HomeQueryService.class);

    @Override
    protected Object initController() {
        return new HomeControllerV2(homeQueryService);
    }

    @Test
    void 최근_댓글_글_인덱스_집계와_캐시_조회() throws Exception {
        when(homeQueryService.getRecentCommentedPostsV2(1L)).thenReturn(List.of(
                new RecentCommentedPostResponse(
                        20L, "최근 대화", LocalDateTime.of(2026, 8, 3, 12, 0)
                )
        ));

        mockMvc.perform(get("/api/v2/home/recent-commented-posts").session(session()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data[0].postId").value(20))
                .andDo(document("home/get-recent-commented-posts-v2",
                        responseFields(

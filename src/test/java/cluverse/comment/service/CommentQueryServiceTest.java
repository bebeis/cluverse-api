...
class CommentQueryServiceTest {
    ...
    void 댓글_목록_조회시_쿼리_결과를_응답으로_조립한다() {
        ...
        when(commentReader.readCommentPage(eq(99L), eq(request), any(CommentPageCursor.class)))
    ...
    @Test
    void 깊은_댓글_문맥은_시작_댓글을_검증한_뒤_스레드_조회로_분리한다() {
        CommentPageRequest request = new CommentPageRequest(10L, null, null, 100);
        Comment root = mock(Comment.class);
        when(commentReader.readOrThrow(101L)).thenReturn(root);
        when(commentReader.readMaxCommentId()).thenReturn(300L);
        when(commentReader.readCommentThreadPage(
                eq(99L), eq(10L), eq(101L), any(CommentPageCursor.class), eq(100)))
                .thenReturn(new CommentPageQueryResult(
                        List.of(createCommentQueryDto(101L, null, 0, false, false)),
                        false,
                        "20260315100000-00000000000000000101"
                ));

        CommentPageResponse response = commentQueryService.getThread(99L, 101L, request);

        assertThat(response.comments()).extracting(CommentResponse::commentId).containsExactly(101L);
        verify(postAccessReader).validateReadablePost(99L, 10L);
        verify(commentReader).validateBelongsToPost(root, 10L);
        verify(commentReader).readCommentThreadPage(
                eq(99L), eq(10L), eq(101L), any(CommentPageCursor.class), eq(100));
    }

...
import cluverse.comment.service.implement.LocalMapCommentWriteProcessor;
import cluverse.comment.service.request.CommentWithPlaceCreateRequest;
...
class LocalMapCommentWriteServiceTest {
    ...
    @Mock
    private LocalMapCommentWriteProcessor processor;
    @Mock
    private LocalMapMetricsRecorder metricsRecorder;
    @Mock
    private PopularityPromotionInvoker popularityPromotionInvoker;
    @InjectMocks
    private LocalMapCommentWriteService service;

    @Test
    void 같은_requestId가_이미_처리됐으면_장소와_댓글을_다시_처리하지_않는다() {
        UUID requestId = UUID.randomUUID();
        CommentWithPlaceCreateRequest request = org.mockito.Mockito.mock(CommentWithPlaceCreateRequest.class);

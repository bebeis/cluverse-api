...
class PopularPostControllerV1DocsTest extends RestDocsSupport {

    private final PopularPostQueryService popularPostQueryService = mock(PopularPostQueryService.class);

    @Override
    protected Object initController() {
        return new PopularPostControllerV1(popularPostQueryService);
    }

    @Test
    void 최근_인기글_V1_조회() throws Exception {
        when(popularPostQueryService.getRecent(PopularityAlgorithmVersion.V1, 20))
        ...
        mockMvc.perform(get("/api/v1/popular-posts/recent").param("size", "20"))
                .andExpect(status().isOk())
                .andDo(document("popular-posts/get-recent-v1",
                        queryParameters(parameterWithName("size").description("조회 개수(1~100)")),
                        responseFields(

...
class PopularPostControllerDocsTest extends RestDocsSupport {

    private final PopularPostQueryService popularPostQueryService = mock(PopularPostQueryService.class);

    @Override
    protected Object initController() {
        return new PopularPostController(popularPostQueryService);
    }

    @Test
    void 과거_인기글_조회() throws Exception {
        when(popularPostQueryService.getHistory(PopularPostSortType.SCORE, 20))
                .thenReturn(new PopularPostListResponse(
                        PopularPostSortType.SCORE,
                        List.of()
                ));

        mockMvc.perform(get("/api/v1/popular-posts/history")
                        .param("sort", "SCORE")
                        .param("size", "20"))
                .andExpect(status().isOk())
                .andDo(document("popular-posts/get-history",
                        queryParameters(

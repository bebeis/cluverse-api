...
- [ ] 앱 기동 확인 — `curl -X POST "$BASE_URL/api/v3/posts/5999999/view-count"` 가 `"code":200` 반환
...
게시글 하나(기본 5999999)에 쓰기를 집중시켜 락 전략별 경합 내성을 비교합니다.
...
| `POST_ID` | `5999999` | fixed 모드 대상 게시글 (05a 핫보드의 ACTIVE 레코드) |
| `POST_ID_MIN` / `POST_ID_MAX` | `5000001` / `5999999` | profile 모드 게시글 범위 (05a 기준) |
...
- 기본 `POST_ID=5999999` 는 05a 의 ACTIVE 핫 레코드(최신 글 6000000은 시드 규칙상 DELETED) = "목록 1페이지 최상단의 핫 레코드" 역할.

...
토큰이 필요 없는 로컬 환경에서는 `BENCHMARK_TOKEN`을 생략할 수 있습니다.
...
`grafana/popularity-dashboard.json`은 설계된 저카디널리티 지표를 사용합니다. Prometheus datasource UID는 `${DS_PROMETHEUS}` 입력으로 연결합니다.

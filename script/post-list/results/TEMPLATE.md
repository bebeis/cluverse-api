...
### 표 2-c — 최종 V3/V4 혼합 부하

`post-list-realistic.k6.js`로 V3 일반 페이지와 V4 날짜·커서 요청을 동시에 발생시킨다.
RATE는 세션 수가 아니라 전체 목표 HTTP RPS다.

| 전체 RATE | V4 목표 비율 | 실측 RPS | 전체 p99 | V3 p99 | V4 진입 p99 | V4 이동 p99 | 에러율 | dropped iterations |
|----------|-------------|----------|----------|--------|-------------|-------------|--------|--------------------|
|          |             |          |          |        |             |             |        |                    |
...
- V4 전체 비율은 `(v4_entry + v4_next) / 전체 요청 수`로 확인한다.
- 전체 p99만으로 원인을 숨기지 않도록 route별 p99를 함께 기록한다.

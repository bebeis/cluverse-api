...
- 정합성: 실패 주입 뒤 누락 metadata가 있는 `COMPLETED`와 오래된 `PENDING`/`COMPENSATING`이 0으로 수렴하는지 본다.
...
`verify_consistency.py`는 DB의 예정 key와 S3 `image-uploads/` prefix를 대조해 누락된 완료 객체, DB에 대응하지 않는 객체, 완료 후 staging 정리 대기, 오래된 미완료 작업 네 숫자만 출력한다. 정상 재조정 이후에는 모두 0이어야 한다.

...
export BASE_URL MEMBER_ID VERSION COMMENTS COMMENTED_POSTS HOT_COMMENT_PERCENT POST_ID RUN_ID
...
K6_ENV=()
for name in BASE_URL MEMBER_ID VERSION COMMENTS COMMENTED_POSTS HOT_COMMENT_PERCENT POST_ID RUN_ID RATE VUS DURATION EXECUTOR REQUEST_TIMEOUT GRACEFUL_STOP PRE_ALLOCATED_VUS MAX_VUS; do
  if [[ -n "${!name:-}" ]]; then
    K6_ENV+=(-e "${name}=${!name}")
  fi
done
